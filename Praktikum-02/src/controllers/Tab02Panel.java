 ...
 * @author N. W. Kusuma
 */
public class Tab02Panel extends JPanel implements ItemListener {
 ...
 */
    ...
    private void dispEach(Object obj){
        JTextField tf = (JTextField) obj;

        if (tf.getText().equals("-")) return;

        if ((!tf.getText().isEmpty() || tf.getText().matches(regex)))
        {
            jtaDisplay.setText(
...
/*
 * Action event Listener for the radio buttons component
 * through super implementation . 
 */
    @Override
...
/*
 * Key event Listeners for the textfields components.
 */
    private void each01KeyReleased(KeyEvent e){
        dispEach(e.getSource());
    }

    private void each02KeyReleased(KeyEvent e){
        dispEach(e.getSource());
    }

    private void each03KeyReleased(KeyEvent e){
        dispEach(e.getSource());
    }
}